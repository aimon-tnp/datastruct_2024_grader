...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      ...
      // initialize pos_itr = begin() + pos
      iterator pos_itr = begin();
      for (int i = 0; i < pos; i++){
        pos_itr++;
      }

      // initialize selected_itr (to record itr of selected elements)
      std::vector<iterator> selected_itr(selected.size());
      for (int i = 0; i < selected.size(); i++){
        selected_itr[i] = begin();
        for (int j = 0; j < selected[i]; j++){
          selected_itr[i]++;
        }
      }

      // proceed to insert & erase
      for (auto &e : selected_itr){
          insert(pos_itr,*e);
          erase(e);
      }
    }
