...
void my_recur(int a, int b, CP::map_bst<int,int> &bst){
  if (a > b) return;

  int middle = (a+b) / 2;
  bst[middle] = middle;

  my_recur(a, middle - 1, bst);
  my_recur(middle + 1, b, bst);
}

void gen_best_bst(int n,CP::map_bst<int,int> &bst) { // n = number of elements
