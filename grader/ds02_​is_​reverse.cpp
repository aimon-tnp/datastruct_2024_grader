...
class vector
{
  ...
  public:
    ...
    bool isReverse(const vector<T> &other) const {
      ...
      if (mSize != other.mSize){ // different size
        return false;
      } else {
        if (mSize == 0 && other.mSize == 0){ // both empty
          return true;
        } else {
          for (size_t this_it = 0; this_it <= mSize - 1; this_it++){
            size_t other_it = other.mSize - 1 - this_it;
            if (mData[this_it] != other.mData[other_it]){
              return false;
            }
          }
          return true;
